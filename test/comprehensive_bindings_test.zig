...
test "cuMemGetHandle - get memory handle for device pointer" {
    const count = try cuda.getDeviceCount();
    if (count == 0) return error.SkipZigTest;
    ...
    // Setup: Allocate some device memory first
    const size = @sizeOf(i32) * 100;

    const allocated = try cuda.allocDeviceMemory(size);
    defer _ = cuda.freeDeviceMemory(allocated) catch {};

    // Test getting memory handle using the high-level wrapper
    var handle_result: ?*anyopaque = null;
    if (cuda.cuMemGetHandle) |f| {
        const flags: c_uint = 0; // Default flags
        // Cast CUdeviceptr (c_ulonglong) to *const anyopaque for the API call
        // Cast CUdeviceptr (c_ulonglong) to *const anyopaque for the API call
        const dev_ptr_as_const: *const anyopaque = @ptrCast(&allocated);
        const res = f(@ptrCast(&handle_result), flags, dev_ptr_as_const, size);

        try testing.expectEqual(@as(i32, 0), res);
        if (handle_result != null) {
            std.debug.print("✓ cuMemGetHandle succeeded\n", .{});
        }
    } else {
...
test "cuModuleLaunchCooperative - launch cooperative kernels" {
    const count = try cuda.getDeviceCount();
    if (count == 0) return error.SkipZigTest;
    ...
    // Step 1: Create a PTX module with cooperative test kernel
    const ptx_code =
        \\.version 6.0
        \\.target sm_50
        \\.address_size 64
        \\
        \\.visible .entry cooperative_kernel()
        \\{
        \\  ret;
        \\}
    ;
    ...
    if (cuda.cuModuleLoadData) |f| {
        ...
        std.debug.print("✓ Successfully loaded test module for cooperative launch\n", .{});
    } else {
        return error.SkipZigTest;
    }
    ...
    // Step 2: Extract the cooperative kernel function
    var kernel_func: ?*cuda.CUfunction = null;

    if (cuda.cuModuleGetFunction) |f| {
        const func_result = f(&kernel_func, module.?, @ptrCast("cooperative_kernel"));

        if (func_result != 0 or kernel_func == null) {
            std.debug.print("ERROR: Failed to get cooperative function handle\n", .{});
            return error.SkipZigTest;
        }

        std.debug.print("✓ Successfully extracted cooperative kernel function\n", .{});
    } else {
        return error.SkipZigTest;
    }

    // Step 3: Test cuModuleLaunchCooperative
    if (cuda.cuModuleLaunchCooperative != null) {
        const grid_dim_x: c_uint = 1;
        const grid_dim_y: c_uint = 1;
        const block_dim_x: c_uint = 256; // Typical cooperative launch size
        const block_dim_y: c_uint = 1;
        const block_dim_z: c_uint = 1;
        const shared_mem_bytes: c_uint = 0;

        var kernel_params: [32]?*anyopaque = undefined;
        for (&kernel_params) |*param| {
            param.* = null;
        }

        if (cuda.cuModuleLaunchCooperative) |coop_launch_fn| {
            // Test with default stream
            const result = coop_launch_fn(kernel_func.?, grid_dim_x, grid_dim_y, block_dim_x, block_dim_y, block_dim_z, shared_mem_bytes, null, // Default stream
                &kernel_params);

            try testing.expectEqual(@as(i32, 0), result);
            std.debug.print("✓ cuModuleLaunchCooperative with default stream succeeded\n", .{});

            // Step 4: Test cooperative launch with custom stream
            var test_stream: ?*cuda.CUstream = null;
            ...
            if (cuda.cuStreamCreate) |create_fn| {
                ...
                if (create_result == 0 and test_stream != null) {
                    ...
                    // Launch cooperative kernel in custom stream
                    const async_coop_result = coop_launch_fn(kernel_func.?, grid_dim_x, grid_dim_y, block_dim_x, block_dim_y, block_dim_z, shared_mem_bytes, test_stream, // Custom stream
                        &kernel_params);

                    try testing.expectEqual(@as(i32, 0), async_coop_result);
                    ...
                    std.debug.print("✓ cuModuleLaunchCooperative with custom stream succeeded\n", .{});
                }
            } else {
                // If no streams available, that's okay
                std.debug.print("INFO: Stream creation not available for cooperative test\n", .{});
            }
        } else {
            return error.SkipZigTest; // Function pointer missing
        }

        // Step 5: Test with different block sizes typical for cooperative launches
        if (cuda.cuModuleLaunchCooperative) |coop_launch_fn| {
            const coop_configs = [_]struct { x: c_uint, y: c_uint }{ .{ .x = 128, .y = 1 }, .{ .x = 256, .y = 1 }, .{ .x = 512, .y = 1 }, .{ .x = 1024, .y = 1 } };

            for (coop_configs) |config| {
                const config_result = coop_launch_fn(kernel_func.?, grid_dim_x, grid_dim_y, config.x, config.y, block_dim_z, shared_mem_bytes, null, &kernel_params);

                // Cooperative configurations should succeed
                try testing.expectEqual(@as(i32, 0), config_result);
            }

            std.debug.print("✓ cuModuleLaunchCooperative with multiple block sizes succeeded\n", .{});
        }
    } else {
        std.debug.print("cuModuleLaunchCooperative not available on this system\n", .{});
        return error.SkipZigTest;
    }

    // Step 6: Verify cooperative launch behavior differs from regular launch
    if (cuda.cuModuleLaunch != null and cuda.cuModuleLaunchCooperative != null) {
        std.debug.print("INFO: Both cuModuleLaunch and cuModuleLaunchCooperative are available\n", .{});

        // Define test parameters
        const grid_dim_x: c_uint = 1;
        const grid_dim_y: c_uint = 1;
        const block_dim_x: c_uint = 256;
        const block_dim_y: c_uint = 1;
        const block_dim_z: c_uint = 1;
        const shared_mem_bytes: c_uint = 0;
        var kernel_params: [32]?*anyopaque = undefined;
        ...
        if (cuda.cuModuleLaunch) |launch_fn| {
            // Test that both can be called successfully on the same function
            const regular_result = launch_fn(kernel_func.?, grid_dim_x, grid_dim_y, block_dim_x, block_dim_y, block_dim_z, shared_mem_bytes, null, &kernel_params);

            try testing.expectEqual(@as(i32, 0), regular_result);

            // Note: Cooperative launches typically have additional restrictions
            // that we can't fully test without device-specific cooperative launch support
        }
    }
}
